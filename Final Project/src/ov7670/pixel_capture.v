//The objective of this module is to catch the pixels sent by the ov7670
...
module pixel_capture_ov7670(
  ...
  output wire ov_rst,
  output wire ov_pwdn,
  output wire [5:0] pixel, // gray pixel 6
  output wire pixel_valid, //signal for sdram write
  output wire [9:0] row_counter,
  output wire [9:0] col_counter,
  ...
  assign pixel        = gray6_value;
  assign pixel_valid  = pixel_valid_q;
  assign row_counter  = row_counter_q;
  assign col_counter  = col_counter_q;
  assign capture_done = capture_done_q;  
  assign ov_pwdn      = 1'b0;
  assign ov_rst       = 1'b1;

  // buffer for all inputs coming from the camera (for a perfect sync)
...
module pixel_capture_ov7670(
  ...
  // RGB to GRAY conversion
  wire [7:0] r8 = {pixel_q[15:11], 3'b000}; 
  wire [7:0] g8 = {pixel_q[10:5], 2'b00};
  wire [7:0] b8 = {pixel_q[4:0], 3'b000};
  ...
  always @* begin
    ...
    case(state_q) // state of secuential logic
      ...
      idle: begin // waits the start signal and delay the capture
        if (start_capture) begin
          state_d = vsync_nedge;
        end
      end
